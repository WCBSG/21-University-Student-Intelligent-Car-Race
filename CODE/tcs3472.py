...
class TCS3472:
  ...
  def init(self):
    ...
    try:
      ...
      if id_val != 0x44 and id_val != 0x4D:
        print("[TCS] WARN: ID 0x%02X" % id_val)
      ...
      print("[TCS] init EIO: %s (attempt %d)" % (e, self._reinit_count))
  ...
  def _on_i2c_error(self):
    ...
    if ticks_diff(now, self._last_err_ms) > 2000:
      ...
      print("[TCS] I2C EIO x%d (skip frame)" % self._err_n)
    # 连续失败 + 退避时间到 → 尝试重初始化
    if (self._err_n >= 8 and not self._gave_up
      ...
      if self._reinit_count >= 5:
        self._gave_up = True
        print("[TCS] gave up after %d re-init failures" % self._reinit_count)
        return
      print("[TCS] re-init attempt %d (backoff %dms)..." % (self._reinit_count + 1, backoff_ms))
      self.init()
