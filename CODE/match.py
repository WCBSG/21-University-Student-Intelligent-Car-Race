...
from robot import IDLE, TRACK, COMPLETE, FAULT, START_TRACK, STOP, ABORT
...
class MatchRunner:
  ...
  def _enter_push(self):
    self._robot.handle(STOP)          # ★ 与 ORBIT 一致：先停再抢 MATCH
    self._arb.acquire(self.OWNER)
    self._approach_deadline = 0
    self._phase_ms = ticks_ms()
  ...
  def _lock_active_class(self, cls_id):
    """锁定当前目标类别，禁止 ORBIT/APPROACH 改跟 allow 内其它类。"""
    self._active_cls = int(cls_id)
    self._cfg.tracking.target_class = self._active_cls
    self._cfg.match_allow = None
