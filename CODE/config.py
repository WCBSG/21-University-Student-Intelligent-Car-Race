...
match_target_count = 2           # 要推的物体总数（得分 N 次后回库）
...
push_duty = 70.0                 # PUSH 推物直行速度
leave_shift_duty = 50.0          # LEAVE SHIFT 横向平移速度
push_correct_duty = 40.0         # PUSH 过偏时慢速纠偏速度
...
push_cx_left_min = 50.0          # 目标 cx 下限（%，超出视为偏左）
push_cx_right_max = 52.0         # 目标 cx 上限（%，超出视为偏右）
...
orbit_front_slip = 100.0         # 绕前方轴侧移满额速度（增大绕行半径）
...
orbit_confirm_frames = 1         # 居中确认帧数 → CLOSE
