...
### Otto 运动学实验 (分支 otto-kinematics)
- `MotionControlOtto`: HP Otto Omni3 公式，支持 `move_with_spin(speed, angle, spin)` 一体化
- 测试脚本: `CODE/test_otto.py` — `smooth_test(30)` 并排对比原版 vs Otto
- 轮序映射: `set_map(0,1,2)` 调 `_MAP`，方向不对时重排轮序
- Otto 右绕轴: `move_with_spin(25, -90, -15)` → [大, 0, 0] 单轮驱动绕轴
